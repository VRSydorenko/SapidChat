//
//  BalanceVC.m
//  SapidChat
//
//  Created by Viktor Sydorenko on 12/3/12.
//  Copyright (c) 2012 Viktor Sydorenko. All rights reserved.
//

#import "BalanceVC.h"
#import "UserSettings.h"
#import "CurrentProModeCell.h"
#import "LocalizationUtils.h"
#import "Lang.h"
#import "ReadAboutProCell.h"
#import "MakeItProCell.h"
#import "BalanceGroupHeaderCell.h"
#import "CurrentBalanceCell.h"
#import "DataManager.h"
#import "BuyOptionCell.h"

@interface BalanceVC ()

@end

@implementation BalanceVC
@synthesize tableContent;

- (void)viewDidLoad
{
    [super viewDidLoad];
    self.title = [Lang LOC_BALANCE_SCREEN_TITLE];
    self.tableContent.dataSource = self;
    self.tableContent.delegate = self;
}

- (void)viewDidUnload
{
    [self setTableContent:nil];
    [super viewDidUnload];
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    switch (section) {
        case 0: // Pro or not Pro
            return [UserSettings premiumUnlocked] ? 2 : 3; // state and info [plus buy button]
        case 1: // Balance state
            return 2;
        case 2: // purchases
            return 4; // 1 header and 3 options
    }
    return 0;
}

- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    return 3; // current mode, balance state, purchases
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.section) {
        case 0:
            switch (indexPath.row) {
                case 0:{
                    CurrentProModeCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CellProOrNotPro"];
                    NSString* text = [UserSettings premiumUnlocked] ? [Lang LOC_BALANCE_PRO_YES] : [Lang LOC_BALANCE_PRO_NO];
                    [LocalizationUtils setText:text forLabel:cell.labelCurrentProMode];
                    return  cell;
                }
                case 1:{
                    ReadAboutProCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CellReadMoreAboutPro"];
                    [LocalizationUtils setText:[Lang LOC_BALANCE_WHAT_IN_PRO] forLabel:cell.labelReadAboutPro];
                    return cell;
                }
                case 2:{
                    MakeItProCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CellMakeItPro"];
                    [cell.segmentedControl setTitle:[Lang LOC_BALANCE_BTN_MAKEPRO] forSegmentAtIndex:0];
                    [cell.segmentedControl setTitle:[Lang LOC_BALANCE_BTN_RESTORE] forSegmentAtIndex:1];
                    return cell;
                }
            }
            break;
        case 1:
            switch (indexPath.row) {
                case 0:{
                    BalanceGroupHeaderCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CellGroupHeader"];
                    [LocalizationUtils setText:[Lang LOC_BALANCE_HEADER_YOUR_BALANCE] forLabel:cell.labelHeaderTitle];
                    return cell;
                }
                case 1:{
                    CurrentBalanceCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CellCurrentBalance"];
                    int poststamps = [DataManager getRegularPoststampsCount] + [DataManager getRegularPoststampsFromLocalBuffer];
                    int tail = poststamps%10;
                    NSString* text;
                    if (poststamps%100 == 11){
                        text = [Lang LOC_BALANCE_POSTSTAMPS_5];
                    } else if (tail == 1){
                        text = [Lang LOC_BALANCE_POSTSTAMPS_1];
                    } else if (tail >= 1 && tail <= 3){
                        text = [Lang LOC_BALANCE_POSTSTAMPS_3];
                    } else {
                        text = [Lang LOC_BALANCE_POSTSTAMPS_5];
                    }
                    text = [NSString stringWithFormat:@"%d %@", poststamps, text];
                    [LocalizationUtils setText:text forLabel:cell.labelCurrentBalance];
                    return cell;
                }
            }
            break;
        case 2:
            switch (indexPath.row) {
                case 0:{
                    BalanceGroupHeaderCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CellGroupHeader"];
                    [LocalizationUtils setText:[Lang LOC_BALANCE_HEADER_TOPUP] forLabel:cell.labelHeaderTitle];
                    return cell;
                }
                case 1:
                case 2:
                case 3:{
                    BuyOptionCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CellBuyOption"];
                    cell.labelQuantity.text = indexPath.row == 1 ? @"10" : indexPath.row == 2 ? @"30" : @"50";
                    cell.labelPercentageOff.text = indexPath.row == 1 ? @"" : indexPath.row == 2 ? @"[-30%]" : @"[-40%]";
                    cell.labelPrice.text = indexPath.row == 1 ? @"$0.99" : indexPath.row == 2 ? @"$1.99" : @"$2.99";
                    return cell;
                }
            }
            break;
    }

    return [[UITableViewCell alloc] init];
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.row) {
        case 0:
            break;
    }
}

@end
